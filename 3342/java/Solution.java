...
class Solution {
    ...
    public static void main(String[] args) {
        ...
        TestCase[] tests = {
                new TestCase(new int[][] { { 0, 4 }, { 4, 4 } }, 7),
                new TestCase(new int[][] { { 0, 0, 0, 0 }, { 0, 0, 0, 0 } }, 6),
                new TestCase(new int[][] { { 0, 1 }, { 1, 2 } }, 4)
                };
         // @formatter:on
    ...
    public int minTimeToReach(int[][] moveTime) {
        ...
        while (!queue.isEmpty()) {
            ...
            for (int[] direction : directions) {
                ...
                int time = Math.max(moveTime[x][y], current[2]) + current[3];
                if (time < times[x][y]) {
                    times[x][y] = time;
                    queue.offer(new int[] { x, y, times[x][y], 3 - current[3] });
                }
            }
        }
        return -1;
