...
class Solution {
    ...
    public static void main(String[] args) {
        ...
        TestCase[] tests = {
                new TestCase(makeTree(new Integer[] { 10, 4, 6 }), true),
                new TestCase(makeTree(new Integer[] { 5, 3, 1 }), false)
                };
         // @formatter:on
        Solution s = new Solution();

        for (TestCase test : tests) {
            boolean actual = s.checkTree(test.in);
            assert test.expected == actual : "checkTree(%s) == %s, want %s".formatted(test.in, actual, test.expected);
        }
    }
